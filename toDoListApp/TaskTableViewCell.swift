...
final class TaskTableViewCell: UITableViewCell {
    ...
    private let descriptionLabel: UILabel = {
        let label = UILabel()
        // ИЗМЕНЕНО: Ограничиваем количество строк двумя
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            // ИЗМЕНЕНО: Выравниваем верх кнопки по верху StackView (где находится заголовок)
            checkboxButton.topAnchor.constraint(equalTo: verticalStackView.topAnchor),
            checkboxButton.widthAnchor.constraint(equalToConstant: 24),
            checkboxButton.heightAnchor.constraint(equalToConstant: 24),
            
            // StackView constraints (остаются без изменений)
    ...
    func configure(with task: Task) {
        ...
        if task.isCompleted {
            ...
            descriptionLabel.textColor = .secondaryLabel // ИЗМЕНЕНО
            ...
            // ГЛАВНОЕ ИСПРАВЛЕНИЕ: Сбрасываем attributedText, чтобы показать обычный текст
            titleLabel.attributedText = nil
            // Возвращаем нормальный цвет заголовку и описанию
            titleLabel.text = task.title
            titleLabel.textColor = .label
            descriptionLabel.textColor = .label // ИЗМЕНЕНО
        }
    ...
    override func prepareForReuse() {
        ...
        // ГЛАВНОЕ ИСПРАВЛЕНИЕ: Сбрасываем и attributedText, и text
        titleLabel.attributedText = nil // ЭТОЙ СТРОКИ НЕ ХВАТАЛО
        titleLabel.text = nil
